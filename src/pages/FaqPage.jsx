...
// ─── FAQ ITEM (Always-visible explorer answer) ───────────────────────────────
function FaqItem({ item, searchQuery }) {
  ...
  return (
    <article className="faq-item" id={`faq-${item.id}`}>
      <h3 className="faq-item-question">{highlightText(item.question)}</h3>
...
const FaqGroup = React.forwardRef(function FaqGroup(
  { category, items, searchQuery, globalIndex },
  ref
) {
